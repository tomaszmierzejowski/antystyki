...
7. **Content Automation (Feature ID: AUTO-GEN-DAILY)**
   - Daily 07:00 local job plus moderator/admin manual trigger to generate 5-6 Statistics and 1-5 Antystics, all `pending_review`
   ...
   - Endpoint `/api/admin/content-generation/run` queues persisted runs; moderators poll `/api/admin/content-generation/runs/{runId}` for `queued/running/succeeded/failed` outcomes and rejection diagnostics
   - Trusted-source guard enforces source reliability floor and optional allowlist; low-confidence extractions are rejected (no fabricated fallback publication)
   ...
   - **Source Yield Governance** (v1.2): each source carries a `StatYield` tier (`high | medium | low`); sources processed in descending yield order; Low-yield sources gated behind `LowYieldSkipThreshold` once enough High/Medium items are fetched; prevents low-density feeds from crowding out finance/economy sources that reliably produce statistical headlines
   - **Extended Pre-Screen Coverage** (v1.2): `PassesPrescreen()` expanded to cover Polish ratio/multiplier/fraction/change phrasings (`1 na 3`, `dwukrotnie`, `wzrósł o 5`, `połowa Polaków`, `jedna trzecia`, etc.) without lowering the factual quality bar
   - **Per-Source Performance Tracking** (v1.2): each completed run persists `SourcePerformanceJson` (fetched/sentToValidation/prescreenPassed/prescreenFailed/validated per source) and `SkippedSourcesJson` in `content_generation_runs` for operational diagnosis and quarantine computation
   - **Auto-Quarantine** (v1.2, opt-in): `SourceQuarantineEnabled=false` by default; when enabled, sources whose rolling pre-screen rate is below `SourceQuarantineMinPrescreenRate` across last `SourceQuarantineWindowRuns` runs are automatically skipped
...
- 2026-03-18T22:30Z: Resolved AUTO-GEN-DAILY `0 valid items` production failure — introduced `StatYield` source tiering with High/Medium/Low fetch ordering and Low-yield gating; expanded `PassesPrescreen()` with 7 new Polish ratio/multiplier/fraction/growth pattern categories; added per-source performance JSON to run records for operational diagnosis; added opt-in rolling quarantine for chronically low-yield sources (§3.1).